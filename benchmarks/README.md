...
Nothing here is imported by Pulse. It runs against the package next to it, or against
an installed one — point `PULSE_SRC` at a checkout to override:

```bash
cd benchmarks
python3 run_limits.py                       # the difficulty ladders
python3 run_detectbench.py                  # the original 67-run suite
python3 edge_cases.py && python3 wiring.py  # hostile input, Keras plumbing
```

Results are written as JSON and are **not** committed — they are regenerated by running
the suites, and the large ones are megabytes of per-case detail that mean nothing apart
from the run that produced them. See `.gitignore`.

`agentbench.py` is the odd one out: it measures the *agent* rather than the detectors,
so it makes real model calls and costs real money. It needs an API key in the
environment (`OPENROUTER_API_KEY`) and takes hours for a full sweep. Its shell mode
gives the audit a read-only terminal in a generated project directory — a capability
Pulse does not ship in its audit path today, so any number from it answers "what could
a scheduled check do with a shell", not "what Pulse does".
