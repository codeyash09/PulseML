...
class PulseCLI:
    ...
    _PERIODIC_CHECKIN_PROMPT = (
        "[Automated Pulse check-in -- sent automatically since it's been {steps} step(s) since "
        "the last one, at roughly {time_per_step} per step] Here is the current state Pulse is "
        "tracking:\n\n{snapshot}\n\n"
        ...
        "NEXTCHECK: <steps until your next check-in should happen -- as low as {min_steps} if "
        "anything looks borderline and worth watching closely, as high as {max_steps} if training "
        "looks stable and boring, or {steps} again if you have no particular reason to change it. "
        "Use the per-step time above to reason in wall-clock terms too if that matters here -- e.g. "
        "a slow, expensive step might warrant a lower step count than a fast one even at the same "
        "level of risk, so you're not left waiting a long time between looks.>"
    )
    ...
    def _maybe_periodic_checkin(self) -> None:
        """Every checkin_interval_steps real training steps (500 by default, but dynamic -- see
        the NEXTCHECK: line in _START_PRIME_PROMPT for the first interval, and the one parsed
        out of this check-in's own reply for every one after) since the last check-in,
        proactively ask the agent two things it would otherwise only ever get asked reactively:
        ...
        It also lets the agent set its own NEXTCHECK: interval (in steps), so a run that looks
        fine can go longer between check-ins and a borderline one can ask to be checked again
        sooner, instead of a fixed cadence -- and, being step-based rather than wall-clock,
        a fast-training run gets reviewed proportionally to how much has actually happened
        instead of on a timer, while a slow one isn't charged a check-in before it's taken more
        than a couple of real steps.
        """
        ...
        prompt = self._PERIODIC_CHECKIN_PROMPT.format(
            ...
            snapshot=snapshot, min_steps=self._CHECKIN_MIN_STEPS, max_steps=self._CHECKIN_MAX_STEPS,
        )
        cprint(
            f"\n[Pulse] ⏱ {self.checkin_interval_steps}-step check-in (step {self.step}, "
            f"{time_per_step}/step) -- asking agent to review training health and GPU tracking...",
            color=_YELLOW,
        )
    ...
    _START_PRIME_PROMPT = (
        ...
        "tracking needs, and to let you re-set this same interval each time) -- decide how many "
